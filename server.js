// ========================
// Servidor Multiplayer Simples para Godot (Versão 3D)
...
// Guarda posição (x,y,z, rotação) e a sala que pertence
const playerlist = {
    players: [],
    ...
    add: function(uuid, roomCode) {
        ...
        // Define posição inicial para o jogador no 3D
        // Jogador 1 começa um pouco para a esquerda (-2), Jogador 2 para a direita (2) no eixo X
        let player = {
            uuid,
            room: roomCode,
            x: isFirstPlayer ? -2.0 : 2.0,
            y: 1.0,  // Altura (para não nascer afundado no chão)
            z: 0.0,  // Profundidade
            r_y: 0.0 // Rotação no eixo Y (para onde está olhando)
        };
    ...
    // Atualiza a posição e rotação de um jogador específico
    update: function(uuid, newX, newY, newZ, newRy) {
...
wss.on("connection", (socket) => {
    ...
    socket.on("message", (message) => {
        ...
        switch (data.cmd) {
            ...
            case "position": {
                // Atualiza posição e rotação do jogador no servidor (Versão 3D)
                playerlist.update(
                    uuid, 
                    data.content.x, 
                    data.content.y, 
